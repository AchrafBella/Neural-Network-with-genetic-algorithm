# -*- coding: utf-8 -*-
"""
Created on Fri Apr  3 19:35:33 2020
...
def fitness(pop, Input):
    output = []
    for elm in pop:
        output.append(sigmoid_derive(np.dot(Input, elm)).tolist() )
    return output
...
def selection(pop, output, real_output):
    i = 0
    for elm in output:
        pop[i] += [mse(elm, real_output)]
        i += 1
    pop.sort( key = lambda accuracy_liste : accuracy_liste[-1])
    for elm in pop:
        del elm[-1]
    return pop
...
def crossover(pop, top):
    ...
    # j'ai supprimer les 10 premiers dans la liste qui contient les 20 synapes
    #  (la population)
    del pop[:top]
